...
class SalesAuthController extends RestfulControllerV2 {
    ...
    public function __construct(SalesAuthServiceInterface $service) {
        $this->middleware('setDealerIdOnRequest')->only(['create', 'update', 'login']);
        $this->middleware('setUserIdOnRequest')->only(['create', 'update', 'login']);
    ...
    public function code(Request $request)
    {
        ...
        if ($request->validate()) {
            // Return Auth
            return $this->response->array($this->service->authorize($request->token_type, $request->auth_code, $request->state, $request->redirect_uri, $request->scopes, $request->sales_person_id));
        }
