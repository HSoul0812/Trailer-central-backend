    ...
    private function findCampaignLeads($dealerId, $campaign)
    {
        ...
        $query = Lead::select('website_lead.*')
                     ...
                     ->leftJoin('crm_text_stop', function($join) {
                        return $join->on(DB::raw("CONCAT('+1', SUBSTR(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(website_lead.phone_number, '(', ''), ')', ''), '-', ''), ' ', ''), '-', ''), '+', ''), 1, 10))"), '=', 'crm_text_stop.sms_number')
                                    ->orOn(DB::raw("CONCAT('+', SUBSTR(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(website_lead.phone_number, '(', ''), ')', ''), '-', ''), ' ', ''), '-', ''), '+', ''), 1, 11))"), '=', 'crm_text_stop.sms_number');
                     })
