...
class SalesLedgerExport extends BaseExportAction implements EntityActionExportable
{
    ...
    public function getQuery()
    {
        ...
        return DB::table('crm_pos_sales as s')
            ->selectRaw("s.id, s.total, s.created_at AS date, c.display_name, sr.receipt_path, groupedRefund.refund_receipts,
            pm.name AS payment_method_name, pm.type AS payment_method_type, COALESCE(groupedRefund.totalOfRefundAmount, 0) AS totalOfRefundAmount,
            s.related_payment_intent AS payment_intent, s.id as pos_sale_id, CONCAT(sp.first_name, ' ', sp.last_name, ' (', sp.email, ')') AS salesPerson,
            s.total as saleTotal, 'pos' as type, s.amount_received as amountReceived, s.id AS refNum")
            ->leftJoin(
