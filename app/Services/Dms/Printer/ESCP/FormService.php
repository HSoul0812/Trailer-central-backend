...
class FormService implements FormServiceInterface 
{
    ...
    private function getFormDR2407(UnitSale $unitSale): array {
        ...
        $this->escpHelper->addText("DR2407", 0, 53);
        $this->escpHelper->addLineBreaks(21);

        // Dealer Name
        $this->escpHelper->addText("Colorado Trailers Inc.", 0, 4);
        $this->escpHelper->addText("DL - 237298", 0, 44);
        $this->escpHelper->addLineBreaks(10);

        // Address
        $this->escpHelper->addText("100 Main Street", 0, 4);
        $this->escpHelper->addText("Springfield", 0, 29);
        $this->escpHelper->addText("TN", 0, 44);
        $this->escpHelper->addText("29021", 0, 53);
        $this->escpHelper->addLineBreaks(10);

        // Inventory to Sell
        $this->escpHelper->addText("1HD1FMW166Y641723", 0, 4);
        $this->escpHelper->addText("2020", 0, 29);
        $this->escpHelper->addText("AMER", 0, 37);
        $this->escpHelper->addText("Alum", 0, 44);
        $this->escpHelper->addText("Rebel", 0, 53);
        $this->escpHelper->addLineBreaks(10);

        // Checkboxes / Date
        $this->escpHelper->makeBold();
        $this->escpHelper->addText("X", 0, 3);
        $this->escpHelper->addText("X", 0, 8);
        $this->escpHelper->addText("X", 0, 14);
        $this->escpHelper->addText("X", 0, 22);
        $this->escpHelper->addText("X", 0, 29);
        $this->escpHelper->addText("X", 0, 36);
        $this->escpHelper->makeBold(false);
        $this->escpHelper->addText("19999.99", 0, 55);
        $this->escpHelper->addLineBreaks(30);

        // Customer
        $this->escpHelper->addText("Jane Doe", 0, 13);
        $this->escpHelper->addText("05/26/2021", 0, 53);
        $this->escpHelper->addLineBreaks(30);
        ...
        // Bottom Row
        $this->escpHelper->addText("n/a", 0, 4);
        $this->escpHelper->addText("05/26/2021", 0, 44);
        $this->escpHelper->addText("DL - 237298", 0, 53);
