...
class AddBuyerbridgeIntegration extends Migration
{
    ...
    private const INTEGRATION_DEALER_BUYERBRIDGE = [
        'integration_id' => 92,
        'dealer_id' => 6549,
        'active' => 1,
        'settings' => [],
    ...
    private const INTEGRATION_SETTINGS = [
        0 => [
            'name' => 'dealer_id',
            'label' => 'Dealer ID',
            'description' => 'Your Buyer Bridge match dealer ID.',
            'type' => 'text',
            'required' => false,
        ],
        1 => [
    ...
    private const DEALER_SETTINGS = [
        'dealer_id' => '6549',
        'host' => 'feeds.buyerbridge.io',
        'username' => 'bb-trailercentral',
        'password' => 'pEh189%OP!%5Okb'
    ];
    ...
    public function up(): void {
        ...
        if ($this->checkIntegration() && !$this->checkIntegrationDealer() && $this->checkDealer()) {
            DB::transaction(function () {
                $integration_dealer = self::INTEGRATION_DEALER_BUYERBRIDGE;
    ...
    private function checkIntegrationDealer(): bool {
        return DB::table(self::DEALER_INTEGRATION_TABLE_NAME)
            ->where('integration_id', self::INTEGRATION_PARAMS['integration_id'])
            ->where('dealer_id', self::INTEGRATION_DEALER_BUYERBRIDGE['dealer_id'])
            ->exists();
    }
    ...
    private function checkDealer(): bool {
        return User::where('dealer_id', self::INTEGRATION_DEALER_BUYERBRIDGE['dealer_id'])->exists();
    }
}
