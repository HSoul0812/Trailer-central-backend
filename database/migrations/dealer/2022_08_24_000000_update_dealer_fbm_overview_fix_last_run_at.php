...
class UpdateDealerFbmOverviewFixLastRunAt extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    private function createView2(): string
    {
        ...
        SELECT 
            ...
            GREATEST(
                ...
                (IFNULL((SELECT created_at FROM fbapp_errors WHERE marketplace_id = fbm.id AND fbapp_errors.inventory_id IS NULL ORDER BY created_at DESC LIMIT 1),'1000-01-01 00:00:00'))
            ) AS last_run_ts,
            (
                (IFNULL((SELECT fbme_listings.listed_at FROM fbme_listings WHERE integration_id = fbm.id ORDER BY fbme_listings.listed_at DESC LIMIT 1),'1000-01-01 00:00:00')) >
                (IFNULL((SELECT created_at FROM fbapp_errors WHERE marketplace_id = fbm.id AND fbapp_errors.inventory_id IS NULL ORDER BY created_at DESC LIMIT 1),'1000-01-01 00:00:00'))
            ...
            IFNULL((SELECT CONCAT(step, ': ', error_message) FROM fbapp_errors WHERE marketplace_id=fbm.id AND DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 4 HOUR)) ORDER BY id DESC LIMIT 1), 'no error') AS error_today,
            IFNULL((SELECT CONCAT(step, ': ', error_message) FROM fbapp_errors WHERE marketplace_id=fbm.id AND DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 28 HOUR)) ORDER BY id DESC LIMIT 1), 'no error') AS error_1dayago,
            IFNULL((SELECT CONCAT(step, ': ', error_message) FROM fbapp_errors WHERE marketplace_id=fbm.id AND DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 52 HOUR)) ORDER BY id DESC LIMIT 1), 'no error') AS error_2dayago,
            IFNULL((SELECT CONCAT(step, ': ', error_message) FROM fbapp_errors WHERE marketplace_id=fbm.id AND DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 76 HOUR)) ORDER BY id DESC LIMIT 1), 'no error') AS error_3dayago,
            IFNULL((SELECT CONCAT(step, ': ', error_message) FROM fbapp_errors WHERE marketplace_id=fbm.id AND DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 100 HOUR)) ORDER BY id DESC LIMIT 1), 'no error') AS error_4dayago,
            IFNULL((SELECT CONCAT(step, ': ', error_message) FROM fbapp_errors WHERE marketplace_id=fbm.id AND DATE(created_at) = DATE(DATE_SUB(NOW(), INTERVAL 124 HOUR)) ORDER BY id DESC LIMIT 1), 'no error') AS error_5dayago
